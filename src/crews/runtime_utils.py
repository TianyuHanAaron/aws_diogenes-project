"""Shared helpers for YAML-configured Nova workflow stages."""
...
async def load_stage_configs(config_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    """Load matching agent/task YAML files for one section helper."""
    return await asyncio.gather(
        load_yaml_async(config_dir / "agents.yaml"),
        load_yaml_async(config_dir / "tasks.yaml"),
    )
...
def build_task_prompt(
    ...
    """Build one direct Nova prompt from YAML-defined agent and task config."""
    parts = [
...
def call_nova_text(prompt: str, *, max_tokens: int = 2600) -> str:
    """Run one synchronous Nova text call."""
...
async def call_nova_text_async(prompt: str, *, max_tokens: int = 2600) -> str:
    """Run one Nova text call without blocking the event loop."""
...
async def run_json_stage(
    ...
    *,
    ...
    """Run one YAML-defined stage and normalize the result to JSON-like data."""
    ...
    if isinstance(payload, (dict, list)):
        return payload
    expected_output = str(task_config.get("expected_output", ""))
    return {} if "JSON object" in expected_output else []
