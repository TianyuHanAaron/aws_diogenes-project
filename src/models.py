"""Shared request, content, and flow-state models for the digest app."""
...
class UserRequest(BaseModel):
    """Normalized input request for one digest run."""
...
class NewsItem(BaseModel):
    """Normalized news record collected from one upstream provider."""
...
class SeasonalEventResult(BaseModel):
    """Raw seasonal-search result bundle kept for the seasonal crew."""
...
class PhotoCandidate(BaseModel):
    """Normalized photo candidate from one image provider."""
...
class CityLandmark(BaseModel):
    """Simple world-webcam link used by the glance section."""
...
class DigestInputs(BaseModel):
    """All fetched inputs required to render one complete digest."""
...
class DigestResult(BaseModel):
    """Rendered digest plus metadata needed for rotation and delivery."""
